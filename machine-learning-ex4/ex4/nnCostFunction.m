...
% Rearranging y to m rows of 10-dimensional vectors
...
J = 1/m.*sum(sum(-y_matrix.*log(h)-(1-y_matrix).*log(1-h))) + ...
    (lambda/(2*m)).*(sum(sum(newTheta1.^2))+ sum(sum(newTheta2.^2)));


d3 = h - y_matrix;

d2 = d3*newTheta2.*sigmoidGradient(X_t*Theta1');
